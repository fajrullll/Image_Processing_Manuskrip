...
# Sakelar manual Freeman Chain Code:
# True  = FCC aktif untuk pemotongan dan visualisasi.
# False = FCC nonaktif; pemotongan dan visualisasi FCC dilewati.
FCC_AKTIF = True
    ...
    for idx, rec in enumerate(freeman_debug_records):
        ...
        if candidate_points_visual is not None and len(candidate_points_visual) > 0:
            ax.scatter(
                ...
                linewidths=1.5,
                zorder=28,
                label="Cut-Point Candidate" if not sudah_label_candidate else "_nolegend_"
            )
        ...
        if cut_col is not None:
            ...
            if tampilkan_cut:
                ax.axvline(
                    ...
                    linestyle="-.",
                    linewidth=2.2,
                    label="FCC Cut Line" if not sudah_label_cut else "_nolegend_",
                    zorder=18
                )
...
image_path = r"E:\progres\GRAPH_FCC_SATUCITRA\CITRA\dengan syafaat rasul.png"
...
# Nilai efektif menjadi None ketika FCC_AKTIF = False.
TARGET_FREEMAN_XRANGE = (290, 320)
target_freeman_efektif = TARGET_FREEMAN_XRANGE if FCC_AKTIF else None
print(f"\nStatus Freeman Chain Code (FCC): {'ON' if FCC_AKTIF else 'OFF'}")
...
hasil_potongan = potong_persimpangan(
    ...
    target_freeman_xrange=target_freeman_efektif,
...
fig.patch.set_facecolor("white")
ax.set_facecolor("black")
ax.imshow(skeleton_used, cmap='gray', vmin=0, vmax=1)
ax.set_aspect('equal', adjustable='box')
ax.set_xlim(0, skeleton_used.shape[1])
ax.set_ylim(skeleton_used.shape[0], 0)

sudah_label_intersection = False
sudah_label_loop = False

for idx_huruf, region_mask in enumerate(sorted_hasil_potongan):
    # Gunakan badan dari hasil segmentasi final agar endpoint sinkron dengan H1-H12.
    badan_huruf_visual = region_mask & (dot_mask == 0)
    skel = skeletonize(badan_huruf_visual)
    endpoints, intersections_visual, _ = find_endpoints(skel)
    loops_visual = deteksi_loop(skel)

    for iy, ix in intersections_visual:
        ax.plot(
            ix, iy, 'o', color='green', markersize=4,
            label='Intersection' if not sudah_label_intersection else '_nolegend_',
            zorder=35
        )
        sudah_label_intersection = True

    for cycle, _ in loops_visual:
        cycle_arr = np.array(cycle)
        ax.plot(
            cycle_arr[:, 1], cycle_arr[:, 0], 'r-', linewidth=2,
            label="Loop" if not sudah_label_loop else "_nolegend_",
            zorder=34
        )
        sudah_label_loop = True

    if len(endpoints) > 0:
        # Arah tulisan Arab adalah kanan ke kiri:
        # start = endpoint paling kanan, end = endpoint paling kiri.
        urutan_endpoint = sorted(
            endpoints,
            key=lambda p: (-int(p[1]), int(p[0]))
        )
        start_y, start_x = urutan_endpoint[0]
        end_y, end_x = urutan_endpoint[-1]

        ax.plot(
            start_x, start_y, 'o', color='blue', markersize=6,
            label='Start Endpoint' if idx_huruf == 0 else "",
            zorder=15
        )
        ax.plot(
...
for dot_idx, yx in enumerate(np.argwhere(dot_mask == 1)):
    ax.plot(
        yx[1], yx[0], 'o', color='magenta', markersize=6,
        label='Upper Diacritic' if dot_idx == 0 else ""
    )
for dot_idx, yx in enumerate(np.argwhere(dot_mask == 2)):
    ax.plot(
        yx[1], yx[0], 'o', color='orange', markersize=6,
        label='Lower Diacritic' if dot_idx == 0 else ""
    )
...
if TAMPILKAN_GARIS_POTONG_AKTUAL:
    ...
    for rec in cut_debug_records:
        ...
        ax.axvline(
            ...
            alpha=0.95,
            zorder=10,
            label="Cut Line" if not sudah_label_garis_potong else "_nolegend_"
        )
...
if FCC_AKTIF and "TARGET_FREEMAN_XRANGE" in globals():
    ax.axvspan(
...
ax.axis('off')
fig.subplots_adjust(left=0.014, right=0.835, top=0.96, bottom=0.04)
fig.savefig(
    os.path.join(OUTPUT_FOLDER, "visualisasi_utama_segmentasi.png"),
    dpi=120,
    facecolor=fig.get_facecolor()
)
plt.close(fig)

# --- VISUALISASI KEY POINT DETECTION ---
# Menampilkan skeleton dan titik-titik fitur tanpa bounding box maupun cut line.
fig_keypoint, ax_keypoint = plt.subplots(
    figsize=(1024 / 120, 215 / 120),
    dpi=120
)
fig_keypoint.patch.set_facecolor("white")
ax_keypoint.set_facecolor("black")
ax_keypoint.imshow(skeleton_used, cmap="gray", vmin=0, vmax=1)
ax_keypoint.set_aspect("equal", adjustable="box")
ax_keypoint.set_xlim(0, skeleton_used.shape[1])
ax_keypoint.set_ylim(skeleton_used.shape[0], 0)

for label, props in component_features.items():
    # Key point detection memakai komponen rasm sebelum proses pemotongan.
    mask_sebelum_potong = (labeled_letters == label) & (dot_mask == 0)
    skel = skeletonize(mask_sebelum_potong)
    endpoints, intersections_keypoint, _ = find_endpoints(skel)
    loops_keypoint = deteksi_loop(skel)

    for point_idx, (iy, ix) in enumerate(intersections_keypoint):
        ax_keypoint.plot(
            ix, iy, "o", color="green", markersize=4,
            label="Intersection" if label == 1 and point_idx == 0 else "",
            zorder=14
        )

    for cycle_idx, (cycle, _) in enumerate(loops_keypoint):
        cycle_arr = np.array(cycle)
        ax_keypoint.plot(
            cycle_arr[:, 1], cycle_arr[:, 0], "r-", linewidth=2,
            label="Loop" if label == 1 and cycle_idx == 0 else "",
            zorder=13
        )

    if len(endpoints) > 0:
        # Arah tulisan Arab: start paling kanan dan end paling kiri
        # pada setiap komponen rasm yang masih belum dipotong.
        urutan_endpoint = sorted(
            endpoints,
            key=lambda p: (-int(p[1]), int(p[0]))
        )
        ...
        ax_keypoint.plot(
            start_x, start_y, "o", color="blue", markersize=6,
            label="Start Endpoint" if label == 1 else "",
            zorder=15
        )
        ax_keypoint.plot(
            end_x, end_y, "o", color="yellow", markersize=6,
...
handles_keypoint, labels_keypoint = ax_keypoint.get_legend_handles_labels()
by_label_keypoint = dict(zip(labels_keypoint, handles_keypoint))
urutan_legend_keypoint = [
    "Intersection",
    "Loop",
    "Upper Diacritic",
    "Lower Diacritic",
...
handles_keypoint_final = [
    by_label_keypoint[nama]
    for nama in urutan_legend_keypoint
    if nama in by_label_keypoint
]
labels_keypoint_final = [
    nama for nama in urutan_legend_keypoint
    if nama in by_label_keypoint
]
...
ax_keypoint.axis("off")
fig_keypoint.subplots_adjust(left=0.014, right=0.835, top=0.96, bottom=0.04)
fig_keypoint.savefig(
    os.path.join(OUTPUT_FOLDER, "visualisasi_key_point_detection.png"),
    dpi=120,
    facecolor=fig_keypoint.get_facecolor()
)
plt.close(fig_keypoint)
...
print("\nMenyimpan visualisasi Biner vs Skeleton+TSP per huruf...")
for idx, region_mask in enumerate(sorted_hasil_potongan):
    ...
    ax[0].set_title(f'Cut Character {idx+1} (Binary)')
    ax[0].axis('off')
    ...
    ax[1].set_title(f'Cut Character {idx+1} (Skeleton + TSP)')
    ax[1].axis('off')
    ...
    plt.tight_layout()
    fig.savefig(
        os.path.join(OUTPUT_FOLDER, f"visualisasi_biner_tsp_huruf_{idx + 1}.png"),
        dpi=200,
        bbox_inches="tight"
    )
    plt.close(fig)
...
print("\nMenyimpan visualisasi Matematis B-Spline per huruf...")
for idx, region_mask in enumerate(sorted_hasil_potongan):
    ...
    # Ukuran tetap mengikuti referensi: 852 x 740 piksel.
    # Figure yang lebih besar membuat kurva, control points, dan legenda lebih jelas.
    fig, ax = plt.subplots(
        figsize=(8.52, 7.40),
        dpi=100
    )
...
for idx, region_mask in enumerate(sorted_hasil_potongan):
    ...
    ax.set_title(f'Character {idx+1}\nDistance: {total_distance:.2f}')
    ax.axis('off')

plt.suptitle("Character Structure + TSP Path + Total Distance")
plt.tight_layout()
fig.savefig(
    os.path.join(OUTPUT_FOLDER, "visualisasi_total_jarak_tsp.png"),
    dpi=200,
    bbox_inches="tight"
)
plt.close(fig)

print(f"\nSemua hasil visualisasi disimpan di: {OUTPUT_FOLDER}")
